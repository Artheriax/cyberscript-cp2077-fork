...
if actionRegion then 
        ...
        function PlayerToggleInvisible(toggle)
                -- B-09 fix: also call UpdateVisibility (matches setInvisible() at npc.lua:1792)
                -- so the engine re-broadcasts the visibility state immediately instead of
                -- relying on the next render tick. Without this, the "infinite invisibility"
                -- option can leave the player permanently invisible after a save/reload
                -- because the saved invisibility flag is never re-synced when the user
                -- disables the option (GH #15).
                local player = GetPlayer()
                if player == nil then return end
                player:SetInvisible(toggle)
...
if vehiculeRegion then 
        ...
        function spawnVehicleV2(chara, appearance, tag, x, y ,z, spawnlevel, spawn_system ,isAV,from_behind,isMP,wait_for_vehicule, scriptlevel, wait_for_vehicle_second,fakeav,despawntimer,persistState,persistSpawn,AlwaysSpawned,SpawnInView,dontregister,rotation)
                -- B-04 / B-11 fix: wrap the entire spawn path in pcall so that any
                -- failure (missing dynamic-entity component, patch 2.3+ vehicle API
                -- change, null entity ref after spawn) is logged and recovered from
                -- instead of crashing the game. The 100% CTD when quickhacking
                -- "Emergency Brakes" on a Cyberscript-spawned vehicle (GH #9) and the
                -- patch 2.3 crash on entering vehicles both originate in this code
                -- path. The pcall doesn't fix the underlying component gap but
                -- prevents the hard crash; the diagnostic log lets users / modders
                ...
                end)  -- close B-04/B-11 pcall anonymous function + call
                if not spawnOk then
                        logme(1, "[Cyberscript] B-04/B-11: spawnVehicleV2 failed for chara=" .. tostring(chara) .. " tag=" .. tostring(tag) .. " err=" .. tostring(spawnErr))
                end
        end
