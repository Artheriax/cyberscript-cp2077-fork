...
function refreshModVariable(active)
        -- B-03 / B-18 fix: throttle to ~20 Hz (every 3 frames at 60 fps).
        -- refreshModVariable recomputes player position, rotation, district,
        -- game_time, money, etc. every frame and is the single biggest per-tick
        -- cost in Cyberscript — compounded by the B-01 GameTime error which
        -- fired every frame and is now fixed. 20 Hz is well above any reasonable
        -- polling rate for UI displays / script-variable reads, so this is
        -- imperceptible to users and downstream mods, but cuts the per-tick
        -- cost by ~3x and eliminates the periodic stutter reported on NexusMods.
        -- The first call after a session load always runs so initial state is
        -- populated immediately.
        refreshModVariableFrameCounter = (refreshModVariableFrameCounter or 0) + 1
        if active == true and refreshModVariableFrameCounter < 3 and refreshModVariableHasRun then
                return
        end
        refreshModVariableFrameCounter = 0
        refreshModVariableHasRun = true
        if active == true then
                ...
                -- so each call passed nil to GetSingleton('GameTime'):Days/Hours/Minutes/Seconds,
                -- which fired the CET type-check error "Function 'Days' parameter 1 must be GameTime."
                -- on every refresh tick. Use the already-populated `currentTime` table instead.
