...
function LoadDataPackCache()
        ...
        -- B-14 fix: apply backward-compatibility aliases for renamed
        -- interactions / menu options. v5.1.4 renamed default interactions
        -- and menu options ("for better clarity" per the changelog) which
        -- silently broke downstream mods that referenced the old names.
        -- This loader reads mod/data/legacy_interaction_aliases.json and
        -- registers each old-name -> new-name mapping as an additional
        -- cache entry pointing at the same data, so legacy JSON actions
        -- continue to fire. Purely additive; no behavior change for mods
        -- using current names.
...
-- tag mapping as an alias in cyberscript.cache["interact"]. If the file is
-- missing or malformed, logs a warning and continues (non-fatal).
function applyLegacyInteractionAliases()
        pcall(function()
                if not file_exists("mod/data/legacy_interaction_aliases.json") then
                        return
                end
                local f = io.open("mod/data/legacy_interaction_aliases.json")
                ...
                for oldTag, newTag in pairs(parsed.aliases) do
                        if type(oldTag) == "string" and type(newTag) == "string" then
                                if cyberscript.cache["interact"] and cyberscript.cache["interact"][newTag] then
                                        cyberscript.cache["interact"][oldTag] = cyberscript.cache["interact"][newTag]
                                        count = count + 1
                                else
                                        logme(2, "[Cyberscript] B-14: alias target '" .. tostring(newTag) .. "' not found for old name '" .. tostring(oldTag) .. "'")
                                end
                        end
                end
                if count > 0 then
                        logme(2, "[Cyberscript] B-14: registered " .. tostring(count) .. " legacy interaction alias(es)")
                end
        end)
end
